...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        const response = await fetch('https://0fa9-14-195-9-98.ngrok-free.app/api/user/fetchUsers');
        if (response.ok) {
          ...
          // if (Array.isArray(userData)) {
            ...
            const currentUser = userData.data.find(user => user.wallets.primary_wallet === address);
